...
//                                'contents' => Storage::get('suspect_cases/' . $suspectCase->id . '.pdf'),
    ...
    // Se realiza un crea muestra_v3 para no modificar el anterior, solamente se modifica y agrega linea de estrategia y subestrategia
    
    public static function crea_muestra_v3(SuspectCase $suspectCase)
    {
        $response = [];
        $client = new Client();
        ...
        try {
            $response = $client->request('POST', env('WS_CREAR_MUESTRA_V3'), [
                'json' => $array,
                'headers'  => [ 'ACCESSKEY' => $suspectCase->laboratory->token_ws]
            ]);
